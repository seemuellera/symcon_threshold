...
class Threshold extends IPSModule {
	...
	protected function CheckIsEqual($inputValue) {
		...
		$this->LogMessage("Running Numerical comparison: Is Equal / value: $inputValue / threshold: " . $this->ReadPropertyFloat("NumericalThreshold"), "DEBUG");
		
		if ($inputValue == $this->ReadPropertyFloat("NumericalThreshold") ) {
		
			$this->LogMessage("Value is equal", "DEBUG");
			$this->UpdateAlertState(true);
		}
		else {
			
			$this->LogMessage("Value is not equal", "DEBUG");
			$this->UpdateAlertState(false);
		}
	}
